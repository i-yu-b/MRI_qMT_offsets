function Mxyf = SPGR_rp(parameters, Wm, offsets, omega1, MT_angles, ...
                        R1f_obs, tau, TR, flip_angle)
%{
...
input:
    ...
    MT_angles - array of flip angles of MT pulse, [rad]
...
% create table Sf(offset, power).
    Sf = compute_Sf(MT_angles,offsets,T2f);
 ...
 %{
  ...
  %}
    ...
    for i=1:num_powers
        for j=1:num_offsets
            S = [Sf(j,i); Sr];
            Mxy(j, i)  = compute_Mxy_rp(kmf, R1m, kfm, R1f, S,  Wm(j,i), TR, flip_angle, tau, M0);  
        end
    end
